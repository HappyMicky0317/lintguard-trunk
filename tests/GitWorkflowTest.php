...
use LintGuardTests\GitFixture;
use LintGuardTests\PhpcsFixture;
use LintGuardTests\TestCache;
use function LintGuard\Cli\runGitWorkflow;
use function LintGuard\GitWorkflow\{isNewGitFile, getGitUnifiedDiff};
...
final class GitWorkflowTest extends TestCase {
	...
	public function testIsNewGitFileReturnsTrueForNewFile() {
		...
		$this->assertTrue(isNewGitFile($gitFile, $git, $executeCommand, array(), '\LintGuardTests\Debug'));
	}
	...
	public function testIsNewGitFileReturnsFalseForOldFile() {
		...
		$this->assertFalse(isNewGitFile($gitFile, $git, $executeCommand, array(), '\LintGuardTests\Debug'));
	}
	...
	public function testGetGitUnifiedDiff() {
		...
		$this->assertEquals($diff, getGitUnifiedDiff($gitFile, $git, $executeCommand, [], '\LintGuardTests\Debug'));
	}
	...
	public function testFullGitWorkflowForOneFileStaged() {
		...
		$messages = runGitWorkflow([$gitFile], $options, $shell, $cache, '\LintGuardTests\Debug');
	...
	public function testFullGitWorkflowForOneFileUnstagedCachesDataThenUsesCache() {
		...
		$cache = new CacheManager( new TestCache(), '\LintGuardTests\Debug' );
		$expected = $this->phpcs->getResults('bin/foobar.php', [20], 'Found unused symbol Foobar.');

		runGitWorkflow([$gitFile], $options, $shell, $cache, '\LintGuardTests\Debug');
	...
	public function testFullGitWorkflowForMultipleFilesStaged() {
		...
		$messages = runGitWorkflow($gitFiles, $options, $shell, $cache, '\LintGuardTests\Debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	}
