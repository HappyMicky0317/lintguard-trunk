...
use LintGuardTests\TestShell;
use LintGuardTests\TestCache;
use LintGuardTests\SvnFixture;
use LintGuardTests\PhpcsFixture;
use function LintGuard\Cli\runSvnWorkflow;
use function LintGuard\SvnWorkflow\{getSvnFileInfo, isNewSvnFile, getSvnUnifiedDiff};
...
final class SvnWorkflowTest extends TestCase {
	...
	public function testIsNewSvnFileReturnsTrueForNewFile() {
		...
		$svnFileInfo = getSvnFileInfo($svnFile, $svn, $executeCommand, '\LintGuardTests\debug');
	...
	public function testGetSvnUnifiedDiff() {
		...
		$this->assertEquals($diff, getSvnUnifiedDiff($svnFile, $svn, $executeCommand, '\LintGuardTests\debug'));
	}
	...
	public function testFullSvnWorkflowForOneFile() {
		...
		$messages = runSvnWorkflow([$svnFile], $options, $shell, new CacheManager(new TestCache()), '\LintGuardTests\debug');
	...
	public function testFullSvnWorkflowForOneFileCached() {
		...
		$messages = runSvnWorkflow([$svnFile], $options, $shell, new CacheManager($cache), '\LintGuardTests\debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	...
	public function testFullSvnWorkflowForOneFileUncachedThenCachesBothVersionsOfTheFile() {
		...
		runSvnWorkflow([$svnFile], $options, $shell, $manager, '\LintGuardTests\debug');
		...
		$messages = runSvnWorkflow([$svnFile], $options, $shell, $manager, '\LintGuardTests\debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	}
	...
	public function testFullSvnWorkflowForOneFileWithCacheOfOldFileVersionDoesNotUseCache() {
		...
		runSvnWorkflow([$svnFile], $options, $shell, new CacheManager($cache), '\LintGuardTests\debug');
	...
	public function testFullSvnWorkflowForMultipleFiles() {
		...
		$messages = runSvnWorkflow($svnFiles, $options, $shell, new CacheManager(new TestCache()), '\LintGuardTests\debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	}
	...
	public function testFullSvnWorkflowForNonSvnFile() {
		...
		$shell->registerCommand("cat 'foobar.php'", $this->phpcs->getResults('STDIN', [20, 99])->toPhpcsJson());
		$options = [];
		runSvnWorkflow([$svnFile], $options, $shell, new CacheManager(new TestCache()), '\LintGuardTests\debug');
	}
